...
import { DIGITALBITS_RPC_URL, XDB_PASSPHRASE } from "../constants";
...
export class XDBService {
	...
	constructor(
		...
		this.server = new Horizon.Server(
			sandbox ? DIGITALBITS_RPC_URL.Sandbox : DIGITALBITS_RPC_URL.Production,
		);
	...
	async transferXDB(amount: string): Promise<string> {
		...
		const transaction = new TransactionBuilder(account, {
			fee: fee.toString(),
			networkPassphrase: this.sandbox ? XDB_PASSPHRASE.Sandbox : XDB_PASSPHRASE.Production,
		})
			.addOperation(
				Operation.payment({
		...
		const tx = TransactionBuilder.fromXDR(
			signedTxXdr,
			this.sandbox ? XDB_PASSPHRASE.Sandbox : XDB_PASSPHRASE.Production,
		);
